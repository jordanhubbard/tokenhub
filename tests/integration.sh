...
check() {
    ...
    local status
    status=$(curl -sf -o /dev/null -w "%{http_code}" -X "$method" "http://localhost:$PORT$url" 2>/dev/null || echo "000")
...
# ──── Health ────
check "GET /healthz returns 200"          GET  /healthz      200
...
# ──── SSE endpoint ────
check "GET /admin/v1/events reachable"    GET  /admin/v1/events  200
